...
$$ N_{i,j,k} \sim Lognormal(\mu_{i,j,k},\tau_i) $$

$$\mu_{i,j,k}<-\alpha_i + \beta_i * abs(Bill_i - Corolla_j) * \beta_2 * Resources_k + \beta_3 * Resources_k * abs(Bill_i - Corolla_j) $$
...
#Source model
source("Bayesian/NmixtureLogNormalRagged.R")

#print model
writeLines(readLines("Bayesian/NmixtureLogNormalRagged.R"))
...
#Parameters to track
ParsStage <- c("alpha","beta1","beta2","beta3","intercept","sigma_int","sigma_slope1","sigma_slope2","sigma_slope3","gamma1","gamma2","gamma3","detect","fit","fitnew","E","sigma_lnorm")

#MCMC options

ni <- 1000  # number of draws from the posterior
nt <- 1  #thinning rate
nb <- ni # number to discard for burn-in
...
m1 = jags(inits=InitStage,
         n.chains=nc,
         model.file="Bayesian/NmixtureLogNormalRagged.jags",
...
```{r}
save.image("Observed_LogNormal.RData")
```
