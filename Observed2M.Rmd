...
```{r}
estl<-pars_detect %>% filter(par="lambda")
indat<-merge(indat,estl,by=c("jBird","jPlant","jTime"))

ggplot(indat,aes(x=Traitmatch,y=BUsed_Flowers,size=estl)) + geom_line() + facet_wrap(~Hummingbird,scales="free",ncol=4)
```
