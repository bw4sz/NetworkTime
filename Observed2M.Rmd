...
#index resources
resourceMatrix<-indat %>% group_by(jBird,jPlant,jTime) %>% summarize(v=unique(FlowerA)) %>% mutate(v=scale(v)) %>% acast(jBird ~ jPlant ~ jTime,value.var='v',fill=0)
...
ggplot(data=spe,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=Resources),alpha=0.2) + geom_line(aes(y=mean,col=Resources),size=.5) + theme_bw() + ylab("Occurrence Probability")+ xlab("Difference between Bill and Corolla Length") + facet_wrap(~Hummingbird,scales="free") + geom_point(data=mindat,aes(x=Traitmatch,y=value,shape=variable),size=1.5,alpha=.75)
...
species.mean<-merge(species.mean,indat[,colnames(indat) %in% c("jBird","jPlant","jTime","Hummingbird","Iplant_Double","BFlowerA")])
...
p<-ggplot(species.mean,aes(x=TotalCorolla,y=lambda,col=as.factor(BFlowerA))) + geom_line(size=.9) + geom_vline(aes(xintercept=Total_Culmen),linetype='dashed') + facet_wrap(~Hummingbird,ncol=3,scales="free_y")  + theme_bw() + ylab("Estimated Daily Interaction Rate") + scale_color_manual("Resource Availability",labels=c("Low","High"),values=c("Blue","Red")) + xlab("Flower Corolla Length (mm)") 
p
...
residuals<-lapply(spres,function(x){
obs<-indat %>% filter(Hummingbird %in% x$Hummingbird,Iplant_Double %in% x$Iplant_Double,BFlowerA %in% x$r)

resids<-c(sapply(na.exclude(c(obs$Camera,obs$Transect)),function(y){
  chisq(o=x$lambda,e=y)
}))

out<-data.frame(chisq=resids,Traitmatch=unique(x$Traitmatch),Hummingbird=unique(x$Hummingbird),Iplant_Double=unique(x$Iplant_Double),r=unique(x$r))
return(out)
})

resdf<-rbind_all(residuals)
ggplot(resdf,aes(x=Traitmatch,y=chisq,col=as.factor(r))) + geom_point() + facet_wrap(~Hummingbird,scales="free") + labs(col="Resource Availability")

ggplot(resdf,aes(x=as.factor(r),y=chisq,fill=as.factor(r))) + geom_boxplot() + facet_wrap(~Hummingbird,scale="free")

resdf %>% group_by(Hummingbird,r) %>% summarize(mean=mean(chisq)) %>% arrange(r) %>% as.data.frame()
```
