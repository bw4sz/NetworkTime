...
datph$Survey_Type[!mt=="Transect"]<-"Camera"
...
for(x in 1:dim(indatlong)[4]){
  ...
  if(length(pres)>0){
    for (y in 1:length(pres)){
      ...
      cam<-as.numeric(dimnames(indatlong)[[4]][x])
...
Timelookup<-indatraw %>% dplyr::select(Time,DateP,Transect_R,Month,Year) %>% group_by(Camera,DateP,Transect_R,Month,Year) %>% distinct() %>% arrange(Time)
