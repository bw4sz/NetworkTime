...
InitStage <- function(){
  ...
  initY<-array(dim=c(Birds,Plants,Times),100)
...
#Jags
  ...
  Nobs=nrow(indat)
  cam_surveys=(indat$Camera>0)*1
  trans_surveys=(indat$Transect>0)*1
...
ggplot(pars_detect[pars_detect$par %in% c("dcam","dtrans"),],aes(x=estimate)) + geom_histogram() + ggtitle("Posterior Distribution") + theme_bw() + facet_wrap(~Hummingbird,ncol=5) + xlab("Probability of Detection")
