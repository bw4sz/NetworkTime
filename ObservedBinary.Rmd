...
```{r}
#match names and Round to 2 decimals
...
```{r}
#Make all responses 0 or 1
obsf$Binary<-(obsf$Yobs>0)*1
```

##Exploratory model fitting

Generalized models for the predictors. Does not account for detection rates, and does not account for non-independence of observations.

##Occurrence as a function of traits

```{r,fig.height=5,fig.width=5}
ggplot(obsf,aes(x=Traitmatch,y=Binary)) + geom_point() + geom_smooth(method="glm",family="binomial")
```

##Occurrence as a function of resources

```{r,fig.height=5,fig.width=5}
ggplot(obsf,aes(x=Flowers,y=Binary)) + geom_point() + geom_smooth(method="glm",family="binomial")
```

##Occurrence as a function of elevation range

```{r,fig.height=5,fig.width=5}
ggplot(obsf,aes(x=ElevD,y=Binary)) + geom_point() + geom_smooth(method="glm",family="binomial")
```
...
tr_interactionF<-glmer(data=obsf,Binary~Traitmatch * scale(Flowers) + (1|Bird) + (1|Plant),family="binomial")

#Interaction of trait and resources as a binary variable
tr_interactionA<-glmer(data=obsf,Binary~Traitmatch * Availability + (1|Bird),family="binomial")

#Interaction of trait and resources as a binary variable
tr_interactionA<-glmer(data=obsf,Binary~Traitmatch * Availability + (1|Bird) ,family="binomial")

#Full model
full<-glmer(data=obsf,Binary~scale(ElevD) + Traitmatch * scale(Flowers) + (1|Bird) + (1|Plant),family="binomial")
...
Ninit<-rep(max(obsf$Binary),Dat$Nobs)

InitStage <- function(){list(beta1=initB,beta2=initB,detect=initB,beta3=initB,alpha=initB,intercept=0,tau_alpha=0.1,tau_beta1=0.1,tau_beta2=0.1,tau_beta3=0.1,gamma1=0,gamma2=0,gamma3=0)}

#Parameters to track
ParsStage <- c("alpha","beta1","beta2","beta3","intercept","sigma_int","sigma_slope1","sigma_slope2","sigma_slope3","gamma1","gamma2","gamma3","detect","fit","fitnew","true_state")
...
#save model output
save(list="m1",file="Bayesian/SavedModels/OccupancyModelRagged.RData")
```
...
```{r,cache=FALSE,fig.width=12,fig.height=9}
...
psim3<-ggplot(data=predy,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.1,fill="red")  + geom_line(aes(y=mean),size=.8,col="red",linetype="dashed") + theme_bw() + ylab("Interactions") + xlab("Difference between Bill and Corolla Length") + geom_point(data=obsf,aes(x=Traitmatch,y=Binary))
psim3
...
ggplot(data=spe,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.1) + geom_line(aes(y=mean),size=1) + theme_bw() + ylab("Occurrence Probability")+ xlab("Difference between Bill and Corolla Length") + facet_wrap(~Species,scales="free",ncol=3) + geom_point(data=odat,aes(x=Traitmatch,y=Binary))
```
...
```{r}
save.image("ObservedBinary.RData")
```
