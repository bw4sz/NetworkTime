...
true_state<- indat %>% filter(State=="Used") %>% group_by(Hummingbird) %>% summarize(mean=mean(TotalCorolla)) %>% mutate(range=abs(min(TotalCorolla)-max(TotalCorolla)))
...
null_state<- indat %>% filter(State=="Used") %>% group_by(Hummingbird) %>% summarize(mean=mean(TotalCorolla)) %>% mutate(range=abs(max(TotalCorolla)-min(TotalCorolla)))
