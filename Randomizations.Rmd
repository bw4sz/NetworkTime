...
ggplot(nullmonths) + geom_violin(alpha=.8,aes(y=Mean_Null,x=Date_C,group=Date_C))+ geom_point(data=truemonths,aes(y=Mean_True,x=Date_C),col="red",size=1) + theme_bw() + ylab("Corolla Length Selected") + facet_wrap(~Hummingbird,scales="free") + scale_x_datetime(date_breaks = "4 months")
...
ggplot(allmonths) + geom_violin(alpha=.8,aes(y=Mean_Diff,x=as.POSIXct(Date_C),group=Date_C,fill=Mean_Diff>0)) + theme_bw() + ylab("Difference between Bill and Corolla Length") + facet_wrap(~Hummingbird,scales="free",ncol=4)  + labs(fill="Evidence of Trait-matching") + scale_x_datetime(date_breaks = "5 months",date_labels = "%m/%y")
```
