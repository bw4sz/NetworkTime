...
#
ggplot(full.morph,aes(x=MonthA,col=factor(Availability),fill=as.factor(Year),y=TotalCorolla)) + geom_boxplot() + labs(fill="Year",y="Corolla Length (mm)",x=NULL) + scale_fill_manual(values=c("grey60","grey40","grey10")) + theme_bw() + scale_color_manual("Resource Availability",labels=c("Low","High"),values=c("black","red"))
...
b$Billc<-cut(b$Bill,breaks=c(0,20,32,60),labels=c("Short Bill","Medium Bill","Long Bill"))

#try general plot
ggplot(indat[indat$State=="Used",],aes(x=TotalCorolla,fill=as.factor(Availability))) + geom_density(alpha=.7) + facet_wrap(~Hummingbird,scale="free_y",nrow=3) + theme_bw()   + geom_vline(aes(xintercept=Bill.x),linetype="dashed",size=1,col='red') + labs(x=NULL) + scale_fill_manual("Resource Availability",values=c("grey90","black"),labels=c("Low","High"),guide="none") + scale_x_continuous(limits=c(0,60),breaks=c(20,40,60)) + theme(plot.margin=unit(c(.25,.25,0.1,0.1),"cm"))

indat<-merge(indat,b,by="Hummingbird")
...
for (x in 1:length(sindat)){
  p[[x]]<-ggplot(sindat[[x]],aes(x=TotalCorolla,fill=State)) + geom_density(alpha=.7) + facet_grid(Availability~Hummingbird) + theme_bw()  + geom_vline(aes(xintercept=Bill.x),linetype="dashed",size=1,col='red') + scale_fill_manual("",values=c("grey90","black"),labels=c("Non-visited","Visited"),guide="none") + labs(x=NULL)
}
...
for (x in 1:length(sindat)){
  j<-sindat[[x]]
  p[[x]]<-ggplot(j[j$State=="Used",],aes(x=TotalCorolla,fill=as.factor(Availability))) + geom_density(alpha=.7) + facet_wrap(~Hummingbird,scale="free_y",nrow=1) + theme_bw()   + geom_vline(aes(xintercept=Bill.x),linetype="dashed",size=1,col='red') + labs(x=NULL) + scale_fill_manual("Resource Availability",values=c("grey90","black"),labels=c("Low","High"),guide="none") + scale_x_continuous(limits=c(0,60),breaks=c(20,40,60)) + theme(plot.margin=unit(c(.25,.25,0.1,0.1),"cm"))
}
...
ggsave("Figures/ChangeCorolla_used.svg",height=7,width=9)
ggsave("Figures/ChangeCorolla_used.jpeg",height=7,width=9)
