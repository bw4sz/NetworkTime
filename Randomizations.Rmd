...
thresh<-melt(flower.month %>% group_by(Transect_R) %>%  summarize(Upper=quantile(Flowers,0.66),Lower=quantile(Flowers,0.33)))
...
#Average the two transects per month
  ...
  mflower$Availability<-(mflower$Flowers > quantile(mflower$Flowers,0.66))*1
   mflower$Availability<-factor(mflower$Availability,labels=c("Low","High"))
#     flt<-mflower %>% group_by(Transect_R) %>% summarize(High=quantile(Flowers,0.66),Low=quantile(Flowers,0.33))
# 
