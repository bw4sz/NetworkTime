...
##Account for non-detections

The observed data above only records interactions, but not the lack of interactions. These zeros are valuable pieces of information. I define the species pool by elevation and add in 0's for non-detections. Anything outside of that elevation is considered NA. 
...
indat$Time<-as.factor(indat$Time)

#Subset time vector and remove duplicates
indatraw<-indatraw[,colnames(indatraw) %in% c("Time","Month","Year","DateP","Elev")]
indatraw<-indatraw[duplicated(indatraw),]

#merge with 0's add
obs0<-merge(indat,indatraw,by="Time")

#which elevation bin is each observation within
labs<-paste(seq(1300,2500,200),seq(1500,2700,200),sep="_")
obs0$Transect_R<-cut(obs0$Elev,seq(1300,2700,200),labels=labs)

#remove any duplicates
flower.month<-flower.month[,c("Month","Year","Transect_R","Availability")]

flower.monthD<-flower.month[!duplicated(flower.month),]
obs<-merge(obs0,flower.monthD,by=c("Month","Year","Transect_R"))
```
...
```{r}
ggplot(obs[,],aes(col=Availability,y=Yobs,x=Traitmatch)) + geom_jitter() +  stat_smooth(method="glm",family="poisson")

mod1<-glm(data=obs,family="poisson",Yobs ~ Traitmatch * Availability )
summary(mod1)
```

First thought on sampling, take the mean interaction for each bird, plant combination.

```{r}
mobs<-obs[obs$Yobs > 0,] %>% group_by(Species,Plant,Availability) %>% summarize(Events=n(),d=mean(Yobs),Traitmatch=unique(Traitmatch))
ggplot(mobs,aes(x=Traitmatch,y=d,col=Availability)) + geom_point(size=3,alpha=.8) + geom_smooth(method="glm",family="poisson") + theme_bw() + facet_wrap(~Species,scales="free")
```
...
#Intercept
ggplot(pars[pars$par %in% c("alpha"),],aes(x=Draw,y=estimate,col=as.factor(Chain))) + geom_line() + facet_grid(plant~species,scale="free") + theme_bw() + labs(col="Chain") + ggtitle("Detection Probability")
