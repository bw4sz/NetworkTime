...
for(x in 1:length(sob)){
  sob[[x]]$BUsed_Flowers<-(sob[[x]]$Used_Flowers) >         quantile(sob[[x]]$Used_Flowers,0.5)*1
}
obsf<-rbind_all(sob)
...
InitStage <- function(){list(beta1=initB,beta2=initB,detect=initB-.25,beta3=initB,alpha=initB,intercept=0,tau_alpha=0.1,tau_beta1=0.1,tau_beta2=0.1,tau_beta3=0.1,gamma1=0,gamma2=0,gamma3=0,N=Ninit,dprior=.25)}
...
```{r,fig.height=4}
chainplot(pars,c("gamma1","gamma2","gamma3","sigma_int","sigma_slope1","sigma_slope2","sigma_slope3","dprior","sigma_detect"),"Group Level Regression") + facet_wrap(~par,nrow=1)
```
