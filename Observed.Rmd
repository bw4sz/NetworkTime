...
#visualize
ggplot(obsf,aes(y=Used_Flowers,x=Hummingbird)) + geom_boxplot() + facet_wrap(~BUsed_Flowers,scales = "free",ncol=1) + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=1)) + scale_x_discrete("Resource Availability",labels=c("High","Low")) 
```
...
#visualize
ggplot(obsf,aes(y=FlowerA,x=Hummingbird)) + geom_boxplot() + facet_wrap(~BFlowerA,scales = "free",ncol=1) + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=1)) + scale_x_discrete("Resource Availability",labels=c("High","Low")) 
```
...
```{r}
ggplot(obsf,aes(x=BFlowerA==1,y=Traitmatch,fill=Yobs > 0)) + geom_boxplot() + facet_wrap(~Hummingbird,ncol=2) + labs(x="Resource Availability",fill="Visited") + scale_x_discrete(labels=c("Low","High"))  + ggtitle("Flower Abundance")
...
```{r,fig.height=4}
chainplot(pars,c("gamma1","gamma2","gamma3","sigma_int","sigma_slope1","sigma_slope2","sigma_slope3","dprior","sigma_detect"),"Group Level Regression") + facet_wrap(~par,nrow=1,scales="free")
```
...
#Trajectories from posterior
predH<-trajF(alpha=castdf$intercept,beta1=castdf$gamma1,x=obsf[obsf$BUsed_Flowers==1,"Traitmatch"],resources=obsf[obsf$BUsed_Flowers==1,"Used_Flowers"],beta2=castdf$gamma2,beta3=castdf$gamma3,type='hdi')

predL<-trajF(alpha=castdf$intercept,beta1=castdf$gamma1,x=obsf[obsf$BUsed_Flowers==0,"Traitmatch"],resources=obsf[obsf$BUsed_Flowers==0,"Used_Flowers"],beta2=castdf$gamma2,beta3=castdf$gamma3,type='hdi')

predhl<-melt(list(High=predH,Low=predL),id.vars=colnames(predH))

colnames(predhl)[5]<-"BFlowerL"

obsf$BFlowerL<-factor(as.character(obsf$BUsed_Flowers))
...
#Trajectories from posterior
predH<-trajF(alpha=mean(castdf$intercept),beta1=mean(castdf$gamma1),x=obsf[obsf$BFlowerA==1,"Traitmatch"],resources=obsf[obsf$BFlowerA==1,"Used_Flowers"],beta2=mean(castdf$gamma2),beta3=mean(castdf$gamma3),type='hdi')

predL<-trajF(alpha=mean(castdf$intercept),beta1=mean(castdf$gamma1),x=obsf[obsf$BFlowerA==0,"Traitmatch"],resources=obsf[obsf$BFlowerA==0,"Used_Flowers"],beta2=mean(castdf$gamma2),beta3=mean(castdf$gamma3),type='hdi')
...
for(d in 1:length(species.split)){
  ...
  #scale resources
  species.traj[[d]]<-trajF(alpha=x$alpha,beta1=x$beta1,beta2=x$beta2,beta3=x$beta3,resources=billd$Used_Flowers,x=billd$Traitmatch,type='hdi')
  }
...
for(d in 1:length(species.split)){
  ...
  sl<-trajF(alpha=x$alpha,beta1=x$beta1,beta2=x$beta2,beta3=x$beta3,resources=billd[billd$BFlowerA==0,"Used_Flowers"],x=billd[billd$BFlowerA==0,"Traitmatch"],type='hdi')
  sh<-trajF(alpha=x$alpha,beta1=x$beta1,beta2=x$beta2,beta3=x$beta3,resources=billd[billd$BFlowerA==1,"Used_Flowers"],x=billd[billd$BFlowerA==1,"Traitmatch"],type='hdi')
...
for(d in 1:length(species.split)){
  ...
  species.traj[[d]]<-intF(alpha=dat$alpha,beta1=dat$beta1,x=billd$Traitmatch,resources=billd$Used_Flowers,beta2=dat$beta2,beta3=dat$beta3,type='hdi')
