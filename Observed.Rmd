...
obsf$BUsed_Flowers<-(obsf$Used_Flowers > quantile(obsf$Used_Flowers,0.75))*1
obsf$BFlowerA<-(obsf$FlowerA > quantile(obsf$FlowerA,0.75))*1
```
...
ni <- 2500000  # number of draws from the posterior
nt <- 20  #thinning rate
...
```{r}
#Trajectories from posterior
predH<-trajF(alpha=castdf$intercept,beta1=castdf$gamma1,x=Dat$traitmatch,resources=Dat$resources,beta2=castdf$gamma2,beta3=1,type='hdi')

predL<-trajF(alpha=castdf$intercept,beta1=castdf$gamma1,x=Dat$traitmatch,resources=Dat$resources,beta2=castdf$gamma2,beta3=0,type='hdi')

predhl<-melt(list(High=predH,Low=predL),id.vars=colnames(predH))


ggplot(data=predhl,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=L1),alpha=0.2)  + geom_line(aes(y=mean,col=L1),size=.8) + theme_bw() + ylab("Interactions") + xlab("Difference between Bill and Corolla Length") + geom_point(data=obsf,aes(x=Traitmatch,y=Yobs))+ labs(fill="Resource Availability",col="Resource Availability")
```
...
head(chiorder[,c("Hummingbird","Iplant_Double","chisq","Yobs","BFlowerA","Traitmatch")],10)
