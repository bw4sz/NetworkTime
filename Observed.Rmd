...
#if there were 0 resources, set to 1, for the flower sampled
a<-obsf[obsf$All_Flowers==0,]
```
...
obsf$BAll_Flowers<-(obsf$All_Flowers >quantile(log(obsf$All_Flowers),0.75))*1
...
ggplot(obsf,aes(x=Month %in% c(7,8,9,10,11),y=Traitmatch,fill=Yobs > 0)) + geom_boxplot() + facet_wrap(~Hummingbird,ncol=4) + labs(x="Resource Availability",fill="Visited") + scale_x_discrete(labels=c("Low","High")) + ggtitle("Rainy v Dry")
...
$$ Detect \sim N(dprior,tau_detect) $$
...
$$\sigma_{detect} = \sqrt[2]{\frac{1}{\tau_{detect}}}$$
...
write.csv(obsf,"InputData/FinalObs.csv")
...
InitStage <- function(){list(beta1=initB,beta2=initB,detect=initB-.25,beta3=initB,alpha=initB,intercept=0,tau_alpha=0.1,tau_detect=0.1,tau_beta1=0.1,tau_beta2=0.1,tau_beta3=0.1,gamma1=0,gamma2=0,gamma3=0,N=Ninit,dprior=.25)}

#Parameters to track
ParsStage <- c("alpha","beta1","beta2","beta3","intercept","sigma_int","sigma_slope1","sigma_slope2","sigma_slope3","gamma1","gamma2","gamma3","detect","fit","fitnew","E","dprior","sigma_detect")
...
#Trajectories from posterior
predH<-trajF(alpha=mean(castdf$intercept),beta1=mean(castdf$gamma1),x=obsf[obsf$BAll_Flowers==1,"Traitmatch"],resources=obsf[obsf$BAll_Flowers==1,"BAll_Flowers"],beta2=mean(castdf$gamma2),beta3=mean(castdf$gamma3),type='hdi')

predL<-trajF(alpha=mean(castdf$intercept),beta1=mean(castdf$gamma1),x=obsf[obsf$BAll_Flowers==0,"Traitmatch"],resources=obsf[obsf$BAll_Flowers==0,"BAll_Flowers"],beta2=mean(castdf$gamma2),beta3=mean(castdf$gamma3),type='hdi')
...
for(d in 1:length(species.split)){
  ...
  sl<-trajF(alpha=x$alpha,beta1=x$beta1,beta2=x$beta2,beta3=x$beta3,resources=billd[billd$BAll_Flowers==0,"BAll_Flowers"],x=billd[billd$BAll_Flowers==0,"Traitmatch"],type='hdi')
  sh<-trajF(alpha=x$alpha,beta1=x$beta1,beta2=x$beta2,beta3=x$beta3,resources=billd[billd$BAll_Flowers==1,"BAll_Flowers"],x=billd[billd$BAll_Flowers==1,"Traitmatch"],type='hdi')
