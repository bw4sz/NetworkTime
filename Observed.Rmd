...
Dat <- list(
  ...
  #Length of birds
  Birds=max(obs$Bird),
  Time=as.numeric(obs$Time),
  resources=as.numeric(scale(obs$Flowers)),
  traitmatch=obs$Traitmatch)

#Inits
initB<-rep(.5,Dat$Birds)
Ninit<-array(dim=c(Dat$Birds,max(obs$Plant),max(as.numeric(Dat$Time))),data=10)

InitStage <- function(){list(beta1=initB,detect=initB,beta2=initB,beta3=initB,alpha=initB,intercept=0,tau_alpha=0.1,tau_beta1=0.1,tau_beta2=0.1,tau_beta3=0.1,gamma1=0,gamma2=0,gamma3=0,N=Ninit)}

#Parameters to track
ParsStage <- c("alpha","beta1","beta2","beta3","intercept","sigma_int","sigma_slope1","sigma_slope2","detect","sigma_slope3","gamma1","gamma2","gamma3","Yobs")
