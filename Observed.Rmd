...
#Trajectories from posterior
pred_low<-trajF(alpha=castdf$intercept,beta1=castdf$gamma1,x=Dat$traitmatch,resources=rep(0,length(Dat$resources)),beta2=castdf$gamma2,beta3=castdf$gamma3)

pred_high<-trajF(alpha=castdf$intercept,beta1=castdf$gamma1,x=Dat$traitmatch,resources=rep(1,length(Dat$resources)),beta2=castdf$gamma2,beta3=castdf$gamma3)

predA<-melt(list(Low=pred_low,High=pred_high),id.vars=colnames(pred_low))

ggplot(data=predA,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=L1),alpha=0.4)  + geom_line(aes(y=mean,col=L1),size=.8,linetype="dashed") + theme_bw() + ylab("Interactions") + xlab("Difference between Bill and Corolla Length") + geom_point(data=obsf,aes(x=Traitmatch,y=Yobs)) + labs(fill="Resource Availability")
...
for(d in 1:length(species.split)){
  ...
  Low<-trajF(alpha=x$alpha,beta1=x$beta1,beta2=x$beta2,beta3=x$beta3,resources=rep(0,length(billd$Availability)),x=billd$Traitmatch)
  High<-trajF(alpha=x$alpha,beta1=x$beta1,beta2=x$beta2,beta3=x$beta3,resources=rep(1,length(billd$Availability)),x=billd$Traitmatch)
...
#plot and compare to original data
ggplot(data=spe,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=R),alpha=0.1) + geom_line(aes(y=mean,col=R),size=1) + theme_bw() + ylab("Occurrence Probability")+ xlab("Difference between Bill and Corolla Length") + facet_wrap(~Hummingbird,scales="free",ncol=4) + geom_point(data=obsf,aes(x=Traitmatch,y=Yobs))
```
