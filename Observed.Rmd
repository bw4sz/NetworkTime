...
##Resource availability at each data point - All Resources

How many flowers, of any species, were in bloom within 1 month of sampling event, within the elevation transect.
...
for (x in 1:nrow(indatraw)){
 event_date<-indatraw[x,"DateP"] 
 event_transect<-indatraw[x,"Transect_R"]
 
 #filter by elevation
 byelev<-fltransects[fltransects$Transect_R %in% event_transect$Transect_R,]
 
 bydate<-byelev[abs(as.Date(byelev$DateP) -as.Date(event_date$DateP))  < 14,]
 #count number of flowers
 
 indatraw$All_Flowers[x]<-as.numeric(bydate%>% summarize(n=sum(Total_Flowers,na.rm=T)) %>% .$n)
}
```

##Resource Availability at each data point - utilized resources.
...
#Date, elevation and species list  
  event_date<-indatraw[x,"DateP"] 
 event_transect<-indatraw[x,"Transect_R"]
   sp_list<-slist[slist$Hummingbird %in% obs$Hummingbird[x],"Iplant_Double"]

 #filter by species
  byspecies<-fltransects[fltransects$Iplant_Double %in% sp_list$Iplant_Double,]
  
 #filter by elevation
  byelev<-byspecies[byspecies$Transect_R %in% event_transect$Transect_R,]
 
  #Filter by date
 bydate<-byelev[abs(as.Date(byelev$DateP) -as.Date(event_date$DateP))  < 14,]
 
 #count number of flowers
indatraw$Used_Flowers[x]<-as.numeric(bydate%>% summarize(n=sum(Total_Flowers,na.rm=T)) %>% .$n)
}
```

##Resource Availability at each data point - the abundance of the flower
...
#Resource list for each species.
  ...
  event_date <- indatraw[x,"DateP"]
  event_transect <- indatraw[x,"Transect_R"]
  sp <-  indatraw[x,"Iplant_Double"]
  
  #filter by species
  byspecies <-  fltransects[fltransects$Iplant_Double %in% sp$Iplant_Double,]
  ...
  #count number of flowers
  indatraw$FlowerA[x] <-  as.numeric(bydate %>% summarize(n = sum(Total_Flowers,na.rm = T)) %>% .$n)
  }

ggplot(indatraw,aes(y=FlowerA,x=All_Flowers)) + geom_point()
```
...
obs<-merge(obs,traitmelt,by=c("Hummingbird","Iplant_Double"))
```
...
```{r}
mobs<-obs[,] %>% group_by(Hummingbird,Iplant_Double,Availability) %>% summarize(Events=n(),d=mean(Yobs),Traitmatch=unique(Traitmatch))

ggplot(mobs,aes(x=Traitmatch,y=d,col=as.factor(Availability))) + geom_point(aes(size=Events),alpha=.8) + theme_bw() + facet_wrap(~Hummingbird,scales="free") + geom_line(aes(group=Iplant_Double),col="black") + scale_size_continuous(range=c(3,7)) + labs(col="Flower Availability")
```

##Intensity as a function of resources

```{r}
mobs<-obs[,] %>% group_by(Hummingbird,Iplant_Double,Month,Transect_R) %>% summarize(Events=n(),d=mean(Yobs),Traitmatch=unique(Traitmatch),Flowers=mean(Flowers))

ggplot(mobs,aes(x=Flowers,y=d,col=Traitmatch)) + geom_point(aes(size=Events),alpha=.8) + theme_bw() + facet_wrap(~Hummingbird,scales="free") + scale_color_continuous(low="blue",high="red") + geom_smooth(method="glm",family="poisson")
```

##Intensity as a function of elevation range

```{r}
mobs<-obs[,] %>% group_by(Hummingbird,Iplant_Double) %>% summarize(Events=n(),d=mean(Yobs),Traitmatch=unique(Traitmatch),ElevD=mean(ElevD))

ggplot(mobs,aes(x=ElevD,y=d,col=Traitmatch)) + geom_point(aes(size=Events),alpha=.8) + theme_bw() + facet_wrap(~Hummingbird,scales="free") + scale_color_continuous(low="blue",high="red") + geom_smooth(method="glm",family="poisson") + geom_text(aes(label=Iplant_Double),size=3)
```
...
indatmerge<-merge(indat,Timelookup[,!colnames(Timelookup) %in% c("Bird","Plant")],by=c("Time","Transect_R"),all.x=T)
...
#Date, elevation and species list  
 ...
 #count number of flowers
obsf$Used_Flowers[x]<-as.numeric(bydate%>% summarize(n=sum(Total_Flowers,na.rm=T)) %>% .$n)
}
```
...
#Resource list for each species.
  ...
  #count number of flowers
  obsf$FlowerA[x] <-  as.numeric(bydate %>% summarize(n = sum(Total_Flowers,na.rm = T)) %>% .$n)
  }
```
...
##Intensity as a function of traits
```{r}
mobs<-obsf[,] %>% group_by(Hummingbird,Iplant_Double,Availability) %>% summarize(Events=n(),d=mean(Yobs),Traitmatch=unique(Traitmatch))

ggplot(mobs,aes(x=Traitmatch,y=d,col=as.factor(Availability))) + geom_point(aes(size=Events),alpha=.8) + theme_bw() + facet_wrap(~Hummingbird,scales="free") + geom_line(aes(group=Iplant_Double),col="black") + scale_size_continuous(range=c(2,6)) + labs(col="Flower Availability",y="Average Number of Visits") 
```

##Intensity as a function of resources

```{r}
mobs<-obsf[,] %>% group_by(Hummingbird,Iplant_Double,Month,Transect_R) %>% summarize(Events=n(),d=mean(Yobs),Traitmatch=unique(Traitmatch),Flowers=mean(Flowers))
...
```{r}
ggplot(obsf,aes(x=ElevD,y=Yobs,col=Traitmatch)) + geom_point(alpha=.8) + theme_bw() + facet_wrap(~Bird,scales="free") + scale_color_continuous(low="blue",high="red")  + ylab("Number of Visits") + xlim(0,1000) + geom_smooth(method="glm",family="binomial")
```
...
#Parameters to track
ParsStage <- c("alpha","beta1","beta2","beta3","intercept","sigma_int","sigma_slope1","sigma_slope2","sigma_slope3","sigma_slope4","gamma1","gamma2","gamma3","detect","fit","fitnew","E")

#MCMC options

ni <- 50000  # number of draws from the posterior
nt <- 20  #thinning rate
nb <- ni*.8 # number to discard for burn-in
...
head(chiorder[,c("Hummingbird","Iplant_Double","chisq","Yobs","Flowers","ElevD","Traitmatch")],50)
...
ggplot(chi,aes(x=Hummingbird,y=Iplant_Double,fill=log(chisq))) + geom_tile() + theme_bw() + labs(fill="Chi-squared discrepancy") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + scale_fill_gradientn(limits=c(-10,10),colours=c("white","blue","red"))
```
