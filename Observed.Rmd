...
for (x in 1:nrow(obsf)){
 ...
 #filter by elevation
 byelev<-fltransects[fltransects$Transect_R %in% event_transect,]
 
 bydate<-byelev[abs(as.Date(byelev$DateP) -as.Date(event_date))  < 14,]
...
#Date, elevation and species list  
  event_date<-obsf[x,"DateP"] 
  event_transect<-obsf[x,"Transect_R"]
  sp_list<-slist[slist$Hummingbird %in% indatraw$Hummingbird[x],"Iplant_Double"]
...
```{r}
mobs<-obsf[,] %>% group_by(Hummingbird,Iplant_Double,Used_Flowers) %>% summarize(Events=n(),d=mean(Yobs),Traitmatch=unique(Traitmatch))

ggplot(mobs,aes(x=Traitmatch,y=d,col=log(Used_Flowers))) + geom_point(aes(size=Events),alpha=.8) + theme_bw() + facet_wrap(~Hummingbird,scales="free") + geom_line(aes(group=Iplant_Double),col="black") + scale_size_continuous(range=c(2,6)) + labs(col="Flower Availability",y="Average Number of Visits") + scale_ 
```
