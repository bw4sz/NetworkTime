...
for (x in 1:nrow(dath)){
  dath$Bird[x]<-elevH[elevH$Species %in% dath$Hummingbird[x],"Index"]
  dath$Plant[x]<-elevP[elevP$Species %in% dath$Iplant_Double[x],"Index"]
}
...
noid<-dath[is.na(dath$ID),]

id_topaste<-paste(noid$Transect_R,noid$DateP,"Transect",sep="_")
dath[which(is.na(dath$ID)),"ID"]<-id_topaste
  
indatraw<- dath %>% group_by(Bird,Plant,ID,DateP) %>% summarize(Yobs=n(),Elev=mean(ele,na.rm=T)) 
