...
opts_chunk$set(message=FALSE,warning=FALSE,fig.height=8,fig.width=12,echo=FALSE,cache=F, fig.path='figure/',fig.align = "center")
...
levels(int$Hummingbird)[levels(int$Hummingbird) %in% "Crowned Woodnymph"]<-"Green-crowned Woodnymph"
#remove species with less than  20 observations
keep<-names(which(table(int$Hummingbird) > 20))

#Sparkling Violeteat violates closed system?
...
#add unique Day ID
indatraw$Time<-as.numeric(factor(indatraw$ID))

#add day ID
sdat<-split(indatraw,list(indatraw$Time,indatraw$Plant),drop = T)
...
est<-data.frame(Yobs=rnbinom(n=length(indatraw$Yobs),size=moment$estimate[[1]],mu=moment$estimate[[2]]))
...
```{r}
#expand out data
indatlong<-acast(indatraw,Bird~Plant~Time~Day,value.var="Yobs")
...
for(x in 1:dim(indatlong)[3]){
  ...
  if(length(pres)>0){
    for (y in 1:length(pres)){
      ...
      camelev<-indatraw %>% filter(Time==cam) %>% .$Elev %>% unique()
...
colnames(indat)<-c("Bird","Plant","Time","Day","Yobs")
```
...
#transect class
obsf$Transect_R<-as.factor(obsf$Transect_R)
...
```{r}
#temp change to months
allt<-obsf %>% group_by(Transect_R) %>% summarize(Allthresh=quantile(All_Flowers,0.75))

obsf<-merge(obsf,allt,by="Transect_R")
obsf$BAll_Flowers<-obsf$All_Flowers > obsf$Allthresh

#obsf$BAll_Flowers<-(obsf$Month %in% c(6,7,8,9,10,11))*1
```

```{r}
sob<-obsf[obsf$Yobs>0,]
qthresh<-sob %>% group_by(Hummingbird) %>% summarize(UThresh=quantile(Used_Flowers,.75))
obsf<-merge(obsf,qthresh)
obsf$BUsed_Flowers<-(obsf$Used_Flowers > obsf$UThresh)*1

#visualize
ggplot(obsf,aes(y=Used_Flowers,x=Hummingbird)) + geom_boxplot() + facet_wrap(~BUsed_Flowers,scales = "free",ncol=1) + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=1)) 
```

```{r}
#split into each species, get low and high resources, only presence records.
sob<-obsf[obsf$Yobs>0,]
qthresh<-sob %>% group_by(Hummingbird) %>% summarize(Thresh=quantile(FlowerA,.75))
obsf<-merge(obsf,qthresh)
obsf$BFlowerA<-(obsf$FlowerA > obsf$Thresh)*1

#visualize
ggplot(obsf,aes(y=FlowerA,x=Hummingbird)) + geom_boxplot() + facet_wrap(~BFlowerA,scales = "free",ncol=1) + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=1)) 
```

###How many counts for each quantile class?

Do we have enough data?

```{r}
#loop through and calculated number of data points in each quantile cut
tt<-lapply(seq(0,1,.1),function(x) {
  sob<-obsf[obsf$Yobs>0,]

qthresh<-sob %>% group_by(Hummingbird) %>% summarize(AThresh=quantile(FlowerA,x))
om<-merge(obsf,qthresh,by="Hummingbird")
om$BFlowerA<-om$FlowerA > om$AThresh
om<-om[om$Yobs>0 ,]
out<-as.data.frame(table(om$Hummingbird,om$BFlowerA))
colnames(out)<-c("Hummingbird","Resource","Count")
out<-merge(out,qthresh)
return(out)})

names(tt)<-seq(0,1,.1)
tt<-melt(tt,id.vars=colnames(tt[[1]]))
colnames(tt)<-c("Hummingbird","Resources","Count","Flowers","Quantile")
ggplot(tt,aes(col=Resources,x=Quantile,y=Count)) + geom_point(size=3) + geom_line(aes(group=Resources)) + facet_wrap(~Hummingbird,scales='free',ncol=3) +  scale_color_discrete(name="Resource Availability",labels=c("Low","High")) + geom_text(aes(label=round(Flowers)),size.=.8)
```
...
```{r}
counts<-obsf %>% group_by(Hummingbird,BFlowerA,Visited=Yobs>0) %>% summarize(n=length(Yobs)) 
ggplot(data=counts,aes(x=BFlowerA==1,y=n,fill=Visited))+ facet_wrap(~Hummingbird) + geom_bar(data=counts,stat = "identity",position = position_dodge()) + scale_x_discrete("Resource Availability",labels=c("Low","High")) + geom_text(aes(label=n),size=6,position = position_dodge(width=0.9,height=-.25))
```
...
ggplot(mobs,aes(x=Traitmatch,y=d,col=Used_Flowers)) + geom_point(aes(size=Events),alpha=.8)  + scale_size_continuous(range=c(2,6)) + labs(col="Flower Availability",y="Average Number of Visits") + scale_color_continuous(low='blue',high='red') + facet_wrap(~Hummingbird,ncol=2,scales='free') + theme_bw()
```

##Mean Intensity as a function of resources

```{r}
mobs<-obsf[,] %>% group_by(Hummingbird,Iplant_Double,Month,BFlowerA) %>% summarize(Events=n(),d=mean(Yobs),Traitmatch=unique(Traitmatch),Flowers=mean(Used_Flowers))

ggplot(mobs,aes(x=log(Flowers),y=d,col=as.factor(BFlowerA))) + geom_point(aes(size=Events),alpha=.8) + theme_bw() + facet_wrap(~Hummingbird,scales="free") + labs(y="Average Number of Visits",col="Resource Availability")
```

```{r}
#All flowers
ggplot(obsf,aes(x=BAll_Flowers==1,y=Traitmatch,fill=Yobs > 0)) + geom_boxplot() + facet_wrap(~Hummingbird,ncol=3,scale='free') + labs(x="Resource Availability",fill="Visited") + scale_x_discrete(labels=c("Low","High")) + ggtitle("All Flowers")

ggplot(obsf,aes(x=BUsed_Flowers==1,y=Traitmatch,fill=Yobs > 0)) + geom_boxplot() + facet_wrap(~Hummingbird,ncol=4) + labs(x="Resource Availability",fill="Visited") + scale_x_discrete(labels=c("Low","High")) + ggtitle("Used Flowers")

ggplot(obsf,aes(x=BFlowerA==1,y=Traitmatch,fill=Yobs > 0)) + geom_boxplot() + facet_wrap(~Hummingbird,ncol=4) + labs(x="Resource Availability",fill="Visited") + scale_x_discrete(labels=c("Low","High")) + ggtitle("Flower Abundance")

ggplot(obsf,aes(x=Month %in% c(7,8,9,10,11),y=Traitmatch,fill=Yobs > 0)) + geom_boxplot() + facet_wrap(~Hummingbird,ncol=4) + labs(x="Resource Availability",fill="Visited") + scale_x_discrete(labels=c("Rainy","Dry")) + ggtitle("Rainy v Dry")
...
splist<-obsf %>% filter(Yobs>0) %>% group_by(Hummingbird,Resource=BAll_Flowers) %>% distinct(Iplant_Double) %>% dplyr::select(Iplant_Double)
...
for (x in 1:length(splist)){
  ...
  #get index
  if(length(p[[x]])==10){
  in_low<-which(lapply(p[[x]],function(i) i$label) == "Low")

  in_high<-which(lapply(p[[x]],function(i) i$label) == "High")
...
```{r}
fc<-fixed.effects(tr_interactionF)

predy<-trajF(alpha=fc[[1]],beta1=fc[[2]],beta2=fc[[3]],beta3=fc[[4]],x=obsf$Traitmatch,resources =obsf$BFlowerA)

ggplot(data=predy,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.2,fill="red")  +  theme_bw() + ylab("Interactions") + xlab("Difference between Bill and Corolla Length") + geom_point(data=obsf,aes(x=Traitmatch,y=Yobs,col=BFlowerA)) + geom_line(aes(y=mean))
```

### Log tranformed presence only records
```{r}
tr_interactionFL<-lmer(data=obsf[obsf$Yobs>0,],log(Yobs)~Traitmatch * FlowerA + (1|Bird))
summary(tr_interactionFL)
obsflmer<-obsf[obsf$Yobs>0,]
obsflmer$pred<-predict(tr_interactionFL)
ggplot(obsflmer,aes(x=Traitmatch,y=pred,col=as.factor(BFlowerA))) + geom_point() + geom_point(aes(y=log(Yobs)),col='black')
```
...
$$\sigma_{int} = \sqrt[2]{\frac{1}{\tau_\alpha}}$$
$$\sigma_{slope1} = \sqrt[2]{\frac{1}{\tau_\beta1}}$$
$$\sigma_{slope2} = \sqrt[2]{\frac{1}{\tau_\beta2}}$$
$$\sigma_{slope3} = \sqrt[2]{\frac{1}{\tau_\beta3}}$$
...
jTraitmatch<-traitmatchT[rownames(traitmatchT) %in% unique(obsf$Hummingbird),colnames(traitmatchT) %in% unique(obsf$Iplant_Double)]

write.csv(obsf,"InputData/ObservedData.csv")

#scale flowers
obsf$FlowerA<-as.numeric(scale(obsf$FlowerA))
obsf$Used_Flowers<-as.numeric(scale(obsf$Used_Flowers))
obsf$All_Flowers<-as.numeric(scale(obsf$All_Flowers))
```

Create flower input matrix per camera

```{r}
#Get resource value for each camera
resource_matrix<- obsf %>% group_by(jBird,Time) %>% dplyr::select(All_Flowers,BAll_Flowers,Used_Flowers,BUsed_Flowers,FlowerA,BFlowerA) %>% distinct()

#bird by camera matrix
resource_matrix<-melt(resource_matrix,id.vars=c("jBird","Time"),measure.vars="BAll_Flowers")
resource_matrix<-acast(resource_matrix,jBird~Camera,fill=0)
```
...
if(paralleljags){
  ...
  Times=max(as.numeric(obsf$Time))
  Day<-obsf$Day
  Days<-max(obsf$Day)
  Traitmatch=jTraitmatch
  #number of birds to iterate
  Birds=max(obsf$jBird)
  Plants=max(obsf$jPlant)
  Nobs=length(obsf$Yobs)

  #A blank Y matrix - all present
  Ninit<-array(dim=c(Birds,Plants,Cameras),data=max(obsf$Yobs)+1)

  #Inits
  InitStage <- function() {list(beta=rep(0.5,Birds),alpha=rep(0.5,Birds),intercept=0,tau_alpha=0.1,tau_beta=0.1,N=Ninit,gamma=0)}
  
  #Parameters to track
  ParsStage <- c("detect","alpha","beta1","beta2","beta3","intercept","sigma_int","sigma_slope1","sigma_slope2","sigma_slope3","ynew","gamma1","gamma2","gamma3","fit","fitnew")
  
  #MCMC options
  ni <- runs  # number of draws from the posterior
  nt <- 3   #thinning rate
  nb <- runs*.92 # number to discard for burn-in
  nc <- 2  # number of chains

  Dat<-list("Yobs","Bird","Plant","Plants","Traitmatch","Birds","Nobs","Ninit","Day","Days","Camera","Cameras","resources")

    system.time(m1<-do.call(jags.parallel,list(Dat,InitStage,ParsStage,model.file="Bayesian/NmixturePoissonRagged.jags",n.thin=nt, n.iter=ni,n.burnin=nb,n.chains=nc)))
  } else {
  #Input Data
  Dat <- list(
     Yobs=obsf$Yobs,
    ...
    Plant=obsf$jPlant,
    Camera=obsf$Camera,
    Cameras=max(as.numeric(obsf$Camera)),
    Day<-obsf$Day,
    Days<-max(obsf$Day),
    ...
    #A blank Y matrix - all present
  Ninit<-array(dim=c(Dat$Birds,Dat$Plants,Dat$Cameras),data=max(obsf$Yobs)+1)
  ...
  system.time(m1 <- jags(inits=InitStage,
...
runs<-40000
recompile(m1)
m1<-update(m1,n.iter=runs,n.burnin=runs*.9,n.thin=3)
```
...
```{r,fig.height=4,fig.wigth=12}
chainplot(pars,c("gamma1","gamma2","gamma3","sigma_int","sigma_slope1","sigma_slope2","sigma_slope3","dprior","sigma_detect"),"Group Level Regression") + facet_wrap(~par,nrow=2,scales="free")
```
...
#intercept
tracegplot(pars,"detect","Detection Parameters") + facet_wrap(~species,scales='free',ncol=2)
```
...
tracegplot(pars,c("gamma1","gamma2","gamma3","sigma_int","sigma_slope1","sigma_slope2","sigma_slope3","dprior","sigma_detect"),"Group-level Parameters") + facet_wrap(~par,nrow=2,scale='free')
```
...
predy<-trajF(alpha=castdf$intercept,beta1=castdf$gamma1,x=obsf$Traitmatch,resources=obsf$BAll_Flowers,beta2=0,beta3=0,type='hdi')
...
#Trajectories from posterior
predy<-trajF(alpha=castdf$intercept,beta1=castdf$gamma1,x=obsf$Traitmatch,resources=obsf$BAll_Flowers,beta2=castdf$gamma2,beta3=castdf$gamma3,type='hdi')
...
#Trajectories from posterior
predH<-trajF(alpha=castdf$intercept,beta1=castdf$gamma1,x=obsf[obsf$BAll_Flowers==1,"Traitmatch"],resources=obsf[obsf$BAll_Flowers==1,"BAll_Flowers"],beta2=castdf$gamma2,beta3=castdf$gamma3,type='hdi')

predL<-trajF(alpha=castdf$intercept,beta1=castdf$gamma1,x=obsf[obsf$BAll_Flowers==0,"Traitmatch"],resources=obsf[obsf$BAll_Flowers==0,"BAll_Flowers"],beta2=castdf$gamma2,beta3=castdf$gamma3,type='hdi')

predhl<-melt(list(High=predH,Low=predL),id.vars=colnames(predH))

colnames(predhl)[5]<-"BFlowerL"

obsf$BFlowerL<-factor(as.character(obsf$BAll_Flowers))
levels(obsf$BFlowerL)<-c("Low","High")

ggplot(data=predhl,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=BFlowerL),alpha=0.2)  + geom_line(aes(y=mean,col=BFlowerL),size=.8) + theme_bw() + ylab("Interactions") + xlab("Difference between Bill and Corolla Length") + geom_point(data=obsf,aes(x=Traitmatch,y=Yobs))+ labs(fill="Resource Availability",col="Resource Availability") 
```

```{r,fig.height=6,fig.width=10}
#Trajectories from posterior
predH<-trajF(alpha=mean(castdf$intercept),beta1=mean(castdf$gamma1),x=obsf[obsf$BFlowerA==1,"Traitmatch"],resources=obsf[obsf$BFlowerA==1,"BAll_Flowers"],beta2=mean(castdf$gamma2),beta3=mean(castdf$gamma3),type='hdi')

predL<-trajF(alpha=mean(castdf$intercept),beta1=mean(castdf$gamma1),x=obsf[obsf$BFlowerA==0,"Traitmatch"],resources=obsf[obsf$BFlowerA==0,"BAll_Flowers"],beta2=mean(castdf$gamma2),beta3=mean(castdf$gamma3),type='hdi')
...
ggplot(data=predhl,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=BFlowerL),alpha=0.2)  + geom_line(aes(y=mean,col=BFlowerL),size=.8) + theme_bw() + ylab("Interactions") + xlab("Difference between Bill and Corolla Length") + geom_point(data=obsf,aes(x=Traitmatch,y=Yobs))+ labs(fill="Resource Availability",col="Resource Availability") + facet_wrap(~BFlowerL,scales='free')
```

##Visualize interactions

```{r,fig.height=7,fig.width=6}
predyint<-intF(alpha=castdf$intercept,beta1=castdf$gamma1,x=obsf$Traitmatch,resources=obsf$BAll_Flowers,beta2=castdf$gamma2,beta3=castdf$gamma3,type='hdi')
...
for(d in 1:length(species.split)){
  ...
  #scale resources
  species.traj[[d]]<-trajF(alpha=x$alpha,beta1=x$beta1,beta2=x$beta2,beta3=x$beta3,resources=billd$BAll_Flowers,x=billd$Traitmatch,type='hdi')
  }
...
###Species Predictions: High and Low Resolutions
...
#plot and compare to original data
ggplot(data=spe,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=Resources),alpha=0.2) + geom_line(aes(y=mean,col=Resources),size=.5) + theme_bw() + ylab("Occurrence Probability")+ xlab("Difference between Bill and Corolla Length") + facet_wrap(~Hummingbird,scales="free",ncol=3) + geom_point(data=obsf,aes(x=Traitmatch,y=Yobs),size=3)
```
...
for(d in 1:length(species.split)){
  ...
  species.traj[[d]]<-intF(alpha=dat$alpha,beta1=dat$beta1,x=billd$Traitmatch,resources=billd$BAll_Flowers,beta2=dat$beta2,beta3=dat$beta3,type='hdi')
...
#plot and compare to original data
ggplot(data=spe,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=Hummingbird),alpha=0.3) + geom_line(aes(y=mean,col=Hummingbird),size=1) + theme_bw() + xlab("Difference between Bill and Corolla Length")  + ylab("Effect of Resources on Trait Matching") + facet_wrap(~Hummingbird,scales="free",ncol=3)
```
...
```{r}
castdf<-dcast(pars[pars$par %in% c("alpha","beta1","beta2","beta3"),], Chain + Draw+species~par,value.var="estimate")

ggpairs(castdf[,c("alpha","beta1","beta2","beta3")])
```
...
head(chiorder[,c("Hummingbird","Iplant_Double","chisq","Yobs","Used_Flowers","Traitmatch")],10)
