...
colnames(odat)[colnames(odat) %in% "Species.x"]<-"Species"
colnames(odat)[colnames(odat) %in% "Species.y"]<-"Iplant_Double"
...
#plot and compare to original data
ggplot(data=spe,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=R),alpha=0.1) + geom_line(aes(y=mean,col=R),size=1) + theme_bw() + ylab("Occurrence Probability")+ xlab("Difference between Bill and Corolla Length") + facet_wrap(~Species,scales="free",ncol=4) + geom_point(data=odat,aes(x=Traitmatch,y=Yobs))
```
...
#Species names
for (x in 1:nrow(obsf)){
  obsf$Iplant_Double<-elevP[obsf$Plant,"Species"]
  obsf$Species<-elevH[obsf$Bird,"Species"]
}

chiorder<-obsf %>% arrange(desc(chisq),Bird)
head(chiorder[,c("Species","Iplant_Double","chisq","Yobs","Flowers","ElevD","Traitmatch")],50)

#order `by highest average discrepancy
chi<-obsf
ord<-chi %>% dplyr::group_by(Species) %>% summarize(m=mean(chisq)) %>% arrange(desc(m)) %>% dplyr::select(Species) 
chi$Species<-factor(chi$Species,levels=ord$Species)
...
ggplot(chi,aes(x=Species,y=Iplant_Double,fill=log(chisq))) + geom_tile() + theme_bw() + labs(fill="Chi-squared discrepancy") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + scale_fill_gradientn(limits=c(-10,10),colours=c("white","blue","red"))
```
