...
elevP<-merge(pindex,elevP,by.x="Species",by.y="Iplant_Double",all=T)

#add elevation from cameras
...
indatraw<- dath %>% group_by(Bird,Plant,ID,DateP) %>% summarize(Yobs=n(),Elev=mean(ele,na.rm=T))  %>% ungroup() %>% arrange(desc(Yobs))
