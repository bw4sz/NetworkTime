...
keep<-names(which(table(int$Hummingbird) > 35))
...
Dat <- list(
  ...
  resources=as.numeric(scale(obsf$Used_Flowers)),
...
ni <- 350000  # number of draws from the posterior
nt <- 5  #thinning rate
...
for(d in 1:length(species.split)){
  ...
  species.traj[[d]]<-trajF(alpha=x$alpha,beta1=x$beta1,beta2=x$beta2,beta3=x$beta3,resources=billd$Used_Flowers,x=billd$Traitmatch)
  }
...
for(d in 1:length(species.split)){
  ...
  #get data for those species
  billd<-obsf[obsf$Bird %in% index,]
  billd$Used_Flowers<-as.numeric(scale(billd$Used_Flowers))
  
  #Calculate interaction effect
  species.traj[[d]]<-intF(alpha=dat$alpha,beta1=dat$beta1,x=billd$Traitmatch,resources=billd$Used_Flowers,beta2=dat$beta2,beta3=dat$beta3)
...
head(chiorder[,c("Hummingbird","Iplant_Double","chisq","Yobs","Used_Flowers","ElevD","Traitmatch")],10)
