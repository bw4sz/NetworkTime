...
Given that these resource values are very large, scale by dividing by sd and subtracting the mean. Mean =0, sd=1
...
trait_resource<-glmer(data=obsf,Yobs~Traitmatch + Used_Flowers + (1|Bird),family="poisson")

#Interaction of trait and resources 
tr_interactionA<-glmer(data=obsf,Yobs~Traitmatch * All_Flowers + (1|Bird),family="poisson")

tr_interactionU<-glmer(data=obsf,Yobs~Traitmatch * Used_Flowers + (1|Bird),family="poisson")

tr_interactionF<-glmer(data=obsf,Yobs~Traitmatch * FlowerA + (1|Bird),family="poisson")
...
Hold resources at 0, what does trait-matching look like?

```{r,fig.width=9,fig.height=7}
#Trajectories from posterior
predy<-trajF(alpha=castdf$intercept,beta1=castdf$gamma1,x=Dat$traitmatch,resources=Dat$resources,beta2=castdf$gamma2,beta3=0)
...
```{r}
ggplot(pars[pars$par %in% c("detect"),],aes(x=as.factor(species),y=estimate)) + geom_violin() + ggtitle("Estimate of parameters") + theme_bw() + ggtitle("Detection Probability") 
```
