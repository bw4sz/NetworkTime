---
title: "Trait-matching and Available Resources: Generate Network"
...
thresh<-melt(flower.month %>% group_by() %>%  summarize(Upper=quantile(Flowers,0.75)))


flower.month$High<-cut(flower.month$Flowers,c(0,thresh[thresh$variable %in% "Upper","value"],max(flower.month$Flowers)+1),labels=c("Low","High"))

#fix the levels
levels(flower.month$Transect_R)<-c("1300m - 1500m", "1500m - 1700m","1700m - 1900m","1900m - 2100m","2100m - 2300m","2300m - 2500m")

#plot
ggplot(flower.month,aes(x=Month.a,log(Flowers),col=High,shape=as.factor(Year))) + geom_point(size=3) + theme_bw()  + geom_smooth(aes(group=1),col='black',size=.5) + ylab("Flowers") + xlab("Month") + labs(shape="Year", y= "Log Available Flowers") + scale_x_discrete(breaks=month.abb[seq(1,12,2)]) + scale_color_manual(labels=c("Low","High"),values=c("blue","red")) + labs(col="Resource Availability") + facet_wrap(~Transect_R)
...
flt<-mflower %>% group_by() %>% summarize(High=quantile(Flowers,0.5))

mflower$Availability<-cut(mflower$Flowers,c(0,flt$High,max(mflower$Flowers)+1),labels=c("Low","High"))
...
#
ggplot(full.morph,aes(x=TotalCorolla)) + geom_density(aes(fill=Availability),alpha=0.7) + labs(fill="Resource Availability",y="Density",x="Corolla Length (mm)")+ theme_bw() + scale_fill_manual(values=c("Blue","Red"))

ggsave("Figures/CorollaAvailability.jpg",dpi=300,height=3,width=6)
