...
opts_chunk$set(message=FALSE,warning=FALSE,fig.width=5,fig.height=4,echo=TRUE,cache=FALSE,cache.path = 'jp_cache/',fig.align='center',fig.path="figure/")
...
$$\tau_\beta1 \sim \gamma_1i(0.001,0.001)$$
$$\tau_\beta2 \sim \gamma_2i(0.001,0.001)$$
$$\tau_\beta3 \sim \gamma_3i(0.001,0.001)$$
...
```{r}
#Detection figure
ggplot(pars[pars$par %in% c("detect"),],aes(x=factor(species,levels=1:max(as.numeric(species))),y=estimate)) + geom_violin(fill='black') + ggtitle("Estimate of parameters") + theme_bw() + ggtitle("Detection Probability") + xlab("Species")
