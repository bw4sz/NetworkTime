...
cat("
    ...
    log(lambda[Bird[x],Plant[x],Time[x]]) <- alpha[Bird[x],Plant[x]] + beta1[Bird[x]] * traitmatch[x] + beta2[Bird[x]] * resources[x] + beta3[Bird[x]] * resources[x] * traitmatch[x]
    ...
    for (j in 1:Plants){
    alpha[i,j] ~ dnorm(intercept,tau_alpha)
    }
    }
