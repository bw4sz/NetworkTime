...
cat(
  ...
  Yobs[x] ~ dbin(detect[Bird[x]],N[Bird[x],Plant[x]])
  N[Bird[x],Plant[x]] ~ dpois(lambda[Bird[x],Plant[x]])
  
  log(lambda[Bird[x],Plant[x]]) <- alpha[Bird[x]] + beta1[Bird[x]] * traitmatch[x] + beta2[Bird[x]] * resources[x] + beta3[Bird[x]] * resources[x] * traitmatch[x]
