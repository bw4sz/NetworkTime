...
sink("Bayesian/NmixtureLogNormalRagged.jags")
...
cat("
    ...
    mu[Bird[x],Plant[x],Time[x]] <- alpha[Bird[x]] + beta1[Bird[x]] * traitmatch[x] + beta2[Bird[x]] * resources[x] + beta3[Bird[x]] * resources[x] * traitmatch[x]      
    
    #True State
    N[x] ~ dlnorm(mu[Bird[x],Plant[x],Time[x]],tau[Bird[x]])    
    ...
    #transform log normal precision to variance
    sigma_lnorm[i] <- pow(1/tau[i],0.5)
    }
