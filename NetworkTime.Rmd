...
```{r}
overallmetrics<-networklevel(as.matrix(tweb),level="higher",index=c("weighted connectance","weighted nestedness"))
...
for(x in 1:length(null_all$perm)){
  nm<-networklevel(null_all$perm[[x]],level="higher",index=c("weighted connectance","weighted nestedness"))
...
num<-droplevels(null_metrics[null_metrics$Metric %in% c( "weighted connectance","weighted nestedness"),])

ob<-droplevels(overallmetrics[overallmetrics$Metric %in% c("weighted connectance","weighted nestedness"),])
...
true_stat<-networklevel(month.inter,index=c("weighted connectance","weighted nestedness"),level="higher")
...
r.int<-permatfull(month.inter,times=500)

#Create a functi\on that computes network metrics
null_total<-as.data.frame(
  t(sapply(r.int$perm,function(x){
  #Compute network statistics
    out<-networklevel(x,index=c("weighted connectance","weighted nestedness"),level="higher")
    return(out)
    })))
