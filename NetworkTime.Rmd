...
coeff<-rbind_all(lapply(sdat,function(x){
  ...
  data.frame(Month=unique(x$Month),Year=as.numeric(as.character(unique(x$Year))),cor=mod.c$estimate,p=mod.c$p.value,N=nrow(x))
}))

nStats<-merge(flowerMean,coeff,by=c("Month","Year"))

p<-ggplot(data=nStats,aes(x=log(Flowers),y=cor,shape=as.factor(Year))) + geom_point(aes(size=N)) + geom_smooth(method="lm",aes(group=1)) #+ geom_text(size=3.5,vjust=-.5)
...
p<-p + geom_point(aes(y=cor,shape=as.factor(Year),size=N)) + geom_smooth(method="lm",aes(group=1,y=cor)) 
p<-p+ theme_bw()  + labs(y="Correlaton between Bill and Corolla Length",x="Log Available Resources",shape="Year",y="")

p<-p + geom_text(aes(y=cor,label=month.abb[Month]),size=3.5,vjust=-.5) + scale_y_continuous(limits=c(-.35,.9),breaks=c(-.3,0,.2,.4,.6,.8)) + scale_size_continuous(range=c(2.5,5))
print(p)
```

#To do: Species level specialization
...
month.inter<-table(monthdata$Hummingbird,monthdata$Iplant_Double)
month.interB<-(month.inter>0)*1
...
ggplot(mlist,aes(x=as.factor(Month),y=value,col=Species,shape=Year)) + facet_wrap(~Species) + geom_point(size=3) + geom_line(aes(group=Species)) + theme_bw()

```

```{r,fig.height=9}
h2flowers<-merge(mlist,flowerMean)

ggplot(h2flowers,aes(x=log(Flowers),y=value,col=Species,shape=Year)) + facet_wrap(~Species) + geom_point(size=3) + geom_smooth(aes(group=Species),method="glm",family="binomial") + theme_bw()
```
