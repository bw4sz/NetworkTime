...
```{r}
overallmetrics<-networklevel(as.matrix(tweb),level="lower",index=c("niche overlap","weighted NODF"))
...
for(x in 1:500){
  ...
  nm<-networklevel(nweb,level="lower",index=c("niche overlap","weighted NODF"))
...
num<-droplevels(null_metrics[null_metrics$Metric %in% c( "niche overlap","weighted NODF"),])

ob<-droplevels(overallmetrics[overallmetrics$Metric %in% c("niche overlap","weighted NODF"),])
...
#Set window size
cutm<-"2 month"
int$DateC<-cut(int$TimeStamp,breaks=cutm)
int.M<-split(int,int$DateC,drop = T)
...
null_month<-function(x){
  ...
  #make web
  tweb<-make_web(nint)
  true_state<-melt(networklevel(tweb,level="lower",index=c("niche overlap","weighted NODF")))
...
null_state<-list()
  for(i in 1:500){
    ...
    nint$Hummingbird<-sample(nint$Hummingbird)
    nweb<-make_web(nint)
    ns<-melt(networklevel(nweb,level="lower",index=c("niche overlap","weighted NODF")))
