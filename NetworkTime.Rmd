...
Need to insert the source network script here, probably needs to be updated seperately for the maquipucuna repo.

#Network measures over time
```{r}
#write csv to file
month.Prop<-read.csv("C:/Users/Ben/Dropbox/Thesis/Maquipucuna_SantaLucia/Results/Network/timeMetrics.csv",row.names=1)
...
p<-ggplot(na.omit(m[!(m$Month==12 & m$Year==2014),]),aes(x=Date,y=value,col=Level)) + geom_point(size=3) + geom_line(linetype="dashed",aes(group=Level)) + facet_wrap(~variable,scales="free_y",ncol=1) 
