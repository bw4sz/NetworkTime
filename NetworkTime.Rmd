...
opts_chunk$set(message=FALSE,warning=FALSE,fig.width=13,echo=FALSE)
```
...
```{r}
#Write flower totals
fl.totals<-read.csv("C:/Users/Ben/Dropbox/Thesis/Maquipucuna_SantaLucia/Results/FlowerTransects/FlowerAvailability.csv")
...
# 
# #Flowers at each elevation over time without Karen's data
# ggplot(fl.totals,aes(x=DateP,TotalFlowers,shape=as.factor(Year),col=as.factor(Elev))) + geom_point(size=3) + theme_bw()  + geom_smooth(aes(group=1)) + ylab("Available Flowers") + labs(x="Month",col="Elevation Transect",shape="Year") + scale_x_date(labels = date_format("%m/%y"),breaks= "1 months") 
```
...
```{r}
#write csv to file
month.Prop<-read.csv("TimeMetrics.csv",row.names=1)
...
metricskeep<-c("connectance","H2","cluster.coefficient.HL","niche.overlap.HL")
...
m<-melt(m,measure.vars=c("H2i",metricskeep))

m<-m[!m$variable %in% "H2",]

m<-group_by(m,variable) %>% mutate(stand=value/max(value)) 

#Plot metrics over time
p<-ggplot(m[!(m$Month==12 & m$Year==2014),],aes(x=Date,y=value,col=variable,shape=Level))+ geom_point(size=3) + geom_line() + facet_wrap(~variable,scales="free_y",ncol=1) 
p<-p + theme_bw() + scale_x_date(labels = date_format("%m/%y"),breaks= "2 months") 
p

# # or together
# p<-ggplot(m[!(m$Month==12 & m$Year==2014),],aes(x=Date,y=stand,col=variable,shape=Level))+ geom_point(size=1) + geom_line() 
# p<-p + theme_bw() + scale_x_date(labels = date_format("%m/%y"),breaks= "2 months") 
# p
...
The flower data needs to be much more finely tuned in terms of geographic proximity, what does it matter how many flowers were at at 2500 for an interaction at 1200m.
...
network.fl<-merge(month.totals,m,by=c("Month","Year"))

#Quick visualization 
p<-ggplot(network.fl,aes(Flowers,value,shape=as.factor(Year),col=as.factor(Month))) + facet_wrap(~variable,scale="free") + geom_point(size=3) + geom_smooth(method="lm",aes(group=1)) + theme_bw()
print(p)
