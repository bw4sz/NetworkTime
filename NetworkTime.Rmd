...
ggsave("Figures/AvailableFlowers.jpg",dpi=600,units="in",width=9,height=6)
ggsave("Figures/AvailableFlowers.svg",units="in",width=9,height=6)
...
network.fl$HL<- network.fl$Flowers > quantile(network.fl$Flowers,0.5)

keep<-c("mean.number.of.shared.partners.HL","niche.overlap.HL","weighted NODF","weighted connectance")

ggplot(network.fl[network.fl$Metric %in% keep,],aes(x=HL,y=True)) + geom_boxplot() + facet_wrap(~Metric,scales="free")  + scale_x_discrete("Resource Availability",labels=c("Low","High"))  + ylab("value")

ggsave("Figures/Boxplot_ResourcesSubset.jpg",height=5,width=10,dpi=300,units="in")
ggsave("Figures/Boxplot_ResourcesSubset.svg",height=5,width=10)
