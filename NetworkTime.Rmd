...
ggplot(fl.totals,aes(x=Month.a,log(TotalFlowers),shape=as.factor(Year))) + geom_point(size=3) + theme_bw()  + geom_smooth(aes(group=1)) + ylab("Flowers") + xlab("Month") + facet_wrap(~Elev,scales="free_y")
...
p<-p+scale_fill_discrete(guide="none") + scale_x_date(breaks="3 months",label=date_format("%m/%y")) + scale_size_continuous(range=c(2.5,5),"Number of Observations")
print(p) 
...
p<-p+ theme_bw()  + labs(y="Correlaton between Bill and Corolla Length)",x="Log Available Resources") + labs(shape="Year")
p<-p + geom_text(aes(y=cor,label=month.abb[Month]),size=3.5,vjust=-.5) + scale_y_continuous(limits=c(-.35,.9),breaks=c(-.3,0,.2,.4,.6,.8))
print(p)
```
...
missing<-colnames(month.interB)[!colnames(month.interB) %in% sp_total$Iplant_Double]

if(length(missing)>0){
print(paste("Abundance missing for species:",missing))
return(NA)}
N_Flowers<-sp_total$N_Flowers
names(N_Flowers)<-sp_total$Iplant_Double

#order 
N_Flowers<-N_Flowers[which(names(N_Flowers) == colnames(month.interB))]

spec<-dfun(month.interB,abuns=sample(N_Flowers))
data.frame(Species=names(spec$dprime),Dprime=spec$dprime,Month=unique(monthdata$Month),Year=unique(monthdata$Year))
return(spec)
}

specmonth(sdat[[10]])
#lapply(sdat,specmonth)
```
