...
```{r}
ggplot(obs[,],aes(col=PrivateResources,y=Yobs,x=Traitmatch)) + geom_point() + scale_color_continuous(low="blue",high="red") + stat_smooth(method="glm",family="poisson") + facet_wrap(~Bird,scale="free")
...
Dat <- list(
  ...
  resources=as.numeric(obs$PrivateResources),
...
for(d in 1:length(species.split)){
  ...
  species.traj[[d]]<-trajF(alpha=x$alpha,beta1=x$beta1,beta2=x$beta2,beta3=x$beta3,resources=Dat$resources,x=Dat$traitmatch)
  }
...
#match colnames
colnames(odat)<-c("Bird","Plant","Time","Date","ID","Yobs","AllResources","PrivateResources","Traitmatch","Species","Iplant_Double")
