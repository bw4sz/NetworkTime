...
coeff<-rbind_all(lapply(sdat,function(x){
  mod.c<-cor(x$Bill,x$TotalCorolla)
...
sizeall<-group_by(m.datH,Month,Year) %>% mutate(Billtype=cut(Bill,breaks=c(0,60),labels=c("all"))) %>% group_by(Month,Year,Billtype) %>% summarize(N=n())
...
#merge with traits
mlistt<-merge(mlist,hum.morph[,colnames(hum.morph) %in% c("Mass","Bill","English")],by.x="Species",by.y="English")

mlistt$BillT<-cut(mlistt$Bill,c(0,30,70),c("Short","Long"))
```
...
p<-ggplot(h2final[!h2final$Iplant_Double %in% "Fuchsia macrostigma",],aes(x=log(Flowers),y=Bill.x-TotalCorolla,col=BillT)) + geom_smooth(method="lm") + geom_point() + geom_text(aes(label=paste(Species,Iplant_Double)),size=2)
p<-p + theme_bw()
p
```
...
flowerh<-lapply(hum,function(h){
  ...
  full.fl %>% filter(Iplant_Double %in% levels(factor(plants$Iplant_Double))) %>% group_by(Month,Year,Transect_R,Date_F) %>% dplyr::summarise(Flowers=sum(Total_Flowers,na.rm=TRUE))  %>% filter(Flowers>2) %>% mutate(Hummingbird=h) %>% group_by(Month,Year,Transect_R,Hummingbird) %>% summarize(TFlowers=mean(Flowers,na.rm=T)) %>% filter(TFlowers>30)
  })
...
#remove species with less than 5 records

p<-ggplot(idflower,aes(x=Hummingbird,y=(Bill-TotalCorolla) + Bill*.5,fill=log(Flowers)>9)) + geom_boxplot()
p<-p + theme_bw() + geom_smooth(method="lm") + facet_wrap(~Hummingbird,scales="free")
p + scale_fill_discrete("Available Resources",labels=c("Low","High"))
