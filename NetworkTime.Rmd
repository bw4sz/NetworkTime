...
thresh<-melt(flower.month %>% group_by(Transect_R) %>%  summarize(Upper=quantile(Flowers,0.75),Lower=quantile(Flowers,0.50)))

#split and recombine after 

flower.month<-split(flower.month,flower.month$Transect_R)

flower.month<-lapply(flower.month,function(x){
  
  threshr<-thresh[thresh$Transect_R %in% x$Transect_R,]
  
  x$High<-cut(x$Flowers,c(0,threshr[threshr$variable %in% "Lower","value"],threshr[threshr$variable %in% "Upper","value"],max(x$Flowers)+1),labels=c("Low","Medium","High"))
  return(x)
})

flower.month<-rbind_all(flower.month)
...
r.int<-permatfull(month.inter,times=50)
...
null_total<-as.data.frame(
  ...
  #Compute network statistics
    out<-networklevel(x,index="quantitative")
    return(out)
    })))
...
network.fl$HL<- cut(network.fl$Flowers,c(0,quantile(network.fl$Flowers,c(0.5,0.75)),max(network.fl$Flowers)+1),labels=c("Low","Medium","High"))
...
ggplot(nsub,aes(x=HL,y=True)) + geom_boxplot() + facet_wrap(~Metric,scales="free") + scale_x_discrete("Resource Availability",labels=c("Low","High"))  + ylab("value")
