...
opts_chunk$set(message=FALSE,warning=FALSE,fig.width=12,echo=FALSE,cache=TRUE,cache.path = 'NetworkTime_cache/', fig.path='figure/')
```
...
```{r,fig.height=7}
#write csv to file
month.Prop<-read.csv("ElevationTimeMetrics.csv",row.names=1)
...
```{r,fig.height=8}
  ...
  full.fl$chunk<-cut(as.numeric(full.fl$Month),c(0,4,9,12),labels=c(2,6,10))
  
  full.fl$Elev<-cut(full.fl$ele,c(0,1700,2500),c("Low","High"))
  
  flower.month<-group_by(full.fl,Month,Elev,Year) %>% dplyr::summarise(Flowers=sum(Total_Flowers,na.rm=TRUE)) %>% filter(!is.na(Elev))
  
  #combine the flower totals and network metrics
  network.fl<-merge(flower.month,m,by=c("Elev","Month","Year"))
  
  #Quick visualization 
  p<-ggplot(network.fl,aes(Flowers,value,shape=as.factor(Year),col=as.factor(Month))) + facet_wrap(Elev~variable,scale="free")  + geom_point(aes(size=Raw.Camera+Raw.Transect),col="black")+ geom_smooth(aes(group=1),method="lm") + theme_bw() + labs(col="Month",shape="Year",size="Interactions") + geom_text(aes(label=Month))
  print(p)
```
