...
```{r}
overallmetrics<-networklevel(as.matrix(tweb),level="lower",index=c("weighted NODF"))
...
for(x in 1:100){
  ...
  nm<-networklevel(nweb,level="lower",index=c("weighted NODF"))
  null_metrics[[x]]<-data.frame(Metric=names(nm),value=nm)
}

null_metrics<-melt(null_metrics)
ggplot(data=null_metrics,aes(x=value)) + geom_density(fill='grey40',alpha=0.5) + facet_wrap(~Metric,scales="free") + geom_vline(data=overallmetrics,aes(xintercept=value),col='red',linetype='dashed',size=1.5) + theme_bw()
```

```{r}
nulls<-ggplot(null_metrics,aes(x=value)) + geom_density(fill='grey40',alpha=0.5) + facet_wrap(~Metric,scales="free",nrow=1) + geom_vline(data=overallmetrics,aes(xintercept=value),col='red',linetype='dashed',size=1.5) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
null_month<-function(x){
  ...
  #make web
  tweb<-make_web(nint)
  true_state<-melt(networklevel(tweb,level="lower",index="weighted NODF"))
...
null_state<-list()
  for(i in 1:100){
    ...
    nint$Hummingbird<-sample(nint$Hummingbird)
    nweb<-make_web(nint)
    ns<-melt(networklevel(nweb,level="lower",index=c("weighted NODF")))
...
```{r}
p<-ggplot(network.fl,aes(x=as.POSIXct(Date_C),fill=Metric)) + facet_wrap(~Metric,scales='free',ncol=1) + geom_linerange(aes(ymin=Lower,ymax=Upper),col="red")
p<-p+ geom_point(aes(y=True_State,col=log(Flowers)),size=7) + geom_line(aes(y=True_State)) 
...
```{r}
p<-ggplot(network.fl,aes(x=as.POSIXct(Date_C),fill=Metric)) + facet_wrap(~Metric,scales='free',ncol=1) + geom_linerange(aes(ymin=True_State-Lower,ymax=True_State-Upper),col="red")
p<-p+ geom_point(aes(y=True_State-Mean,col=log(Flowers)),size=7) + geom_line(aes(y=True_State-Mean)) 
p<-p+scale_fill_discrete(guide="none") + labs(y="",x="Date") + theme_bw() + scale_x_datetime(date_breaks = "2 months",date_labels = "%b %Y") + scale_color_continuous("Available Flowers",high='red',low='blue') 
p
...
```{r}
#Quick visualization 
p<-ggplot(network.fl[,],aes(log(Flowers),y=True_State-Mean,shape=as.factor(Year))) + geom_point(size=7) + facet_wrap(~Metric,scale="free",ncol=3)  + geom_smooth(aes(group=1),method="lm") + theme_bw() + labs(shape="Year",size="Interactions") + geom_text(aes(label=month.abb[as.numeric(Month)]),hjust=-0.25,size=4) + labs(x="Log Available Resources") + ylab("Score") 
p
```
