...
ggplot(flower.month,aes(x=Month.a,log(Flowers),shape=as.factor(Year))) + geom_point(size=3) + theme_bw()  + geom_smooth(aes(group=1)) + ylab("Flowers") + xlab("Month") + facet_wrap(~Transect_R,scales="free_y") + labs(shape="Year")
...
ggplot(h2flowers,aes(x=log(Flowers),y=value,col=BillT,shape=Year)) + facet_wrap(~Species) + geom_point(size=3) + geom_smooth(aes(group=Species),method="glm") + theme_bw()

#match with bill differences
billmatch<-group_by(m.datH,Hummingbird,Month,Year) %>% summarize(BD=mean(BD,na.rm=T),N=n()) %>% filter(N>3) %>% rename(Species=Hummingbird)
h2match<-merge(billmatch,h2flowers)

#merge with original network measures
nm<-dcast(network.fl,Month+Year~Metric,value.var="True")
h2final<-merge(h2match,nm)

p<-ggplot(h2final,aes(x=connectance,y=BD)) + geom_smooth(method="lm") + geom_point()
p + theme_bw()
p 
