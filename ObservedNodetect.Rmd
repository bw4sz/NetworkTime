...
#Hummingbirds not present at that elevation
  ...
  camelev<-    indatraw %>% filter(Time==cam)  %>% .$Elev %>% mean()
...
Timelookup<-indatraw %>% dplyr::select(Time,Transect_R,Month,Year,Day,DateP,ID) %>% unique()

#Get time information
indat<-merge(indat,Timelookup,by=c("Time","Day"))
...
#index resources
resourceMatrix<- indat %>% group_by(jBird,jPlant,jID) %>% summarize(v=mean(BUsed_Flowers)) %>% acast(jBird ~ jPlant ~ jID,value.var='v',fill=0)
...
#print model
writeLines(readLines("Bayesian/NoDetectNmixturePoissonRagged.R"))
...
#Jags
  ...
  m2<-do.call(jags.parallel,list(Dat,InitStage,ParsStage,model.file="Bayesian/NoDetectNmixturePoissonRagged.jags",n.thin=nt, n.iter=ni,n.burnin=nb,n.chains=nc))
```
