...
#add unique Camera and Transect ID
...
```{r}
keep<-indatraw %>% group_by(Hummingbird) %>% filter(Yobs>0) %>% summarise(n=n()) %>% filter(n>10) %>% .$Hummingbird
indat<-indatraw[indatraw$Hummingbird %in% keep,]

#Log transform records
indat$LYobs<-log(indat$Yobs)
```
...
#index resources
resourceMatrix<- indat %>% group_by(jBird,jPlant,jID) %>% summarize(v=unique(BUsed_Flowers)) %>% acast(jBird ~ jPlant ~ jID,value.var='v',fill=0)

#bind to indat for later
mr<-melt(resourceMatrix)
colnames(mr)<-c("jBird","jPlant","jID","scaledR")
indat<-merge(indat,mr,by=c("jBird","jPlant","jID"))
...
$$ Yobs_{i,j,k} \sim Pois(\lambda_{i,j,k}) $$
...
#Source model
source("Bayesian/LogN.R")

#print model
writeLines(readLines("Bayesian/LogN.R"))

#Input Data
Dat <- c('Yobs','Birds','Bird','Plant','Time','Plants','Times','resources','Nobs','Traitmatch',"Day","Days")

#Inits
InitStage <- function(){
  #A blank Y matrix - all present
  initY<-array(dim=c(Birds,Plants,Times),5)
  initB<-as.numeric(matrix(nrow=Birds,ncol=1,data=0))

list(beta1=initB,beta2=initB,beta3=initB,alpha=rep(.5,Birds),intercept=0,tau_alpha=0.1,tau_beta1=0.1,tau_beta2=0.1,tau_beta3=0.1,gamma1=0,gamma2=0,gamma3=0)}
...
#Jags
  ...
  m2<-do.call(jags.parallel,list(Dat,InitStage,ParsStage,model.file="Bayesian/LogN.jags",n.thin=nt, n.iter=ni,n.burnin=nb,n.chains=nc))
```
...
predy<-trajLog(alpha=castdf$intercept,beta1=castdf$gamma1,x=indat$Traitmatch,resources=indat$scaledR,beta2=0,beta3=0,type='hdi')

ggplot(data=predy,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.1,fill="red")  + geom_line(aes(y=mean),size=.5,col="red",linetype="dashed") + theme_bw() + ylab("Interactions") + xlab("Difference between Bill and Corolla Length") + geom_point(data=indat,aes(x=Traitmatch,y=Yobs))
```
...
ggplot(data=predy,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.2,fill="red")  +  theme_bw() + ylab("Interactions") + xlab("Difference between Bill and Corolla Length") + geom_point(data=indat,aes(x=Traitmatch,y=Yobs)) + geom_line(aes(y=mean)) 
```
...
ggplot(data=predhl,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=BFlowerL),alpha=0.2)  + geom_line(aes(y=mean,col=BFlowerL),size=.8) + theme_bw() + ylab("Interactions") + xlab("Difference between Bill and Corolla Length") + geom_point(data=indat,aes(x=Traitmatch,y=Yobs))+ labs(fill="Resource Availability",col="Resource Availability") 
...
ggplot(data=spe,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.2,fill='red') + geom_line(aes(y=mean),size=.5) + theme_bw() + ylab("Occurrence Probability")+ xlab("Difference between Bill and Corolla Length") + facet_wrap(~Hummingbird,scales="free",ncol=4) + geom_point(data=indat,aes(x=Traitmatch,y=Yobs),size=2.5) + labs(shape="Sampling Method")
```
...
#plot and compare to original data
ggplot(data=spe,aes(x=x)) + geom_ribbon(aes(ymin=lower,ymax=upper,fill=Resources),alpha=0.2) + geom_line(aes(y=mean,col=Resources),size=.5) + theme_bw() + ylab("Occurrence Probability")+ xlab("Difference between Bill and Corolla Length") + facet_wrap(~Hummingbird,scales="free",ncol=3) + geom_point(data=indat,aes(x=Traitmatch,y=Yobs),size=1.5,alpha=.5)
...
species.traj<-lapply(species.split,function(dat){
  ...
  for (y in 1:nrow(d)){
    ...
    sampletraj[[y]]<-data.frame(x=as.numeric(billd$Traitmatch),y=as.numeric(v),r=as.numeric(billd$scaledR),jBird=billd$jBird,jPlant=billd$jPlant,jID=billd$jID)
  }
...
```{r}
gc()
save.image("ObservedNodetect.Rdata")
``` 
