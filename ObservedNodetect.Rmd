...
indatraw<- datph %>% group_by(Hummingbird,Iplant_Double,Bird,Plant,ID,DateP,Survey_Type) %>% summarize(Yobs=n(),Month=unique(Month),Year=unique(Year),Elev=mean(ele,na.rm=T),Transect_R=unique(Transect_R))
...
Timelookup<-indatraw %>% dplyr::select(ID,Transect_R,Survey_Type,Month,Year,Day,DateP) %>% group_by(ID,Transect_R,Survey_Type,Day,Month,Year,DateP) %>% distinct() %>% arrange(ID)
